...
class GoogleNet(BaseModel):
    ...
    def _create_conv(self, inputs, data_dict):
        ...
        with arg_scope([conv], trainable=self._trainable, data_dict=data_dict, nl=tf.nn.relu):
            conv1 = conv(inputs, 7, 64, name='conv1_7x7_s2', stride=2)
            pool1 = max_pool(conv1, 'pool1', padding='SAME', filter_size=3, stride=2)
            ...
            pool2 = max_pool(conv2, 'pool2', padding='SAME', filter_size=3, stride=2)
            pool2_lrn = tf.nn.local_response_normalization(pool2, depth_radius=2, alpha=2e-05, beta=0.75, name='pool2_lrn')

        with arg_scope([inception_layer], trainable=self._trainable, data_dict=data_dict):
            inception3a = inception_layer(pool2_lrn, 64, 96, 128, 16, 32, 32, name='inception_3a')
            inception3b = inception_layer(inception3a, 128, 128, 192, 32, 96, 64, name='inception_3b')
            pool3 = max_pool(inception3b, 'pool3', padding='SAME', filter_size=3, stride=2)
            ...
            pool4 = max_pool(inception4e, 'pool4', padding='SAME', filter_size=3, stride=2)
